...
CREATE OR REPLACE FUNCTION auth.staff_company_ids()
RETURNS SETOF uuid
...
CREATE OR REPLACE FUNCTION auth.staff_store_ids()
RETURNS SETOF uuid
LANGUAGE sql SECURITY DEFINER STABLE
AS $$
...
CREATE POLICY "staff_select_own_company" ON companies
  FOR SELECT USING (id IN (SELECT auth.staff_company_ids()));
...
CREATE POLICY "staff_manage_own_company_stores" ON stores
  ...
  USING (company_id IN (SELECT auth.staff_company_ids()))
  WITH CHECK (company_id IN (SELECT auth.staff_company_ids()));
